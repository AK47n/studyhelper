...
const CDP_PORT = Number(process.env.TEST_CDP || 9229)
...
const profile = path.join(os.tmpdir(), `studyhelper-defmode-${CDP_PORT}`)
try {
...
if (!page) {
  ...
  for (let i = 0; i < 60; i++) {
    ...
    if (!st.cover && (st.board || st.note)) break
    await sleep(250)
  }
  ...
  let st2 = null
  for (let i = 0; i < 60; i++) {
    st2 = await s.eval(probe).catch(() => null)
    if (st2 && !st2.cover && (st2.board || st2.note)) break
    await sleep(250)
  }
